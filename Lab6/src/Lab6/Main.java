...
public class Main {
	public static void main(String[] args) throws NoSuchModelNameException, DuplicateModelNameException, IOException, ClassNotFoundException, InstantiationException, IllegalArgumentException, NoSuchMethodException, SecurityException, IllegalAccessException, InvocationTargetException, InterruptedException {
			...
			String output = auto1.toString();
			System.out.println(output);
			/*ThreadForPrices t = new ThreadForPrices(auto1);
			ThreadForModels l = new ThreadForModels(auto1);
			l.setPriority(Thread.MAX_PRIORITY);
			System.out.println();
			t.start();
			l.start();*/
			/*TransportSynchronizer ts = new TransportSynchronizer(auto1);
			Runnable r = new ModelsRunnable(ts);
			Thread mr = new Thread(r);
			Runnable rb = new PricesRunnable(ts);
			Thread pr = new Thread(rb);
			mr.start();
			pr.start();*/
			/*Lock myLock1 = new ReentrantLock();
		   	Runnable run = new LockPrices(auto1, myLock1);
			Thread lp = new Thread(run);
			Runnable runb = new LockModels(auto1, myLock1);
			Thread lm = new Thread(runb);
			lm.start();
			lp.start();*/
		    Vehicle motorcycle1 = new Motorcycle("FirstMark", arg);
			Vehicle scooter1 = new Scooter("SecondMark", arg);
			Vehicle quadBike1 = new QuadBike("ThirdMark", arg);
			Vehicle moped1 = new Moped("LastMark", arg);
			...
			Runnable runAll = new ThreadForAll(array[0]);
			Runnable runAll2 = new ThreadForAll(array[1]);
			Runnable runAll3 = new ThreadForAll(array[2]);
			Runnable runAll4 = new ThreadForAll(array[3]);
			es.submit(runAll);
			es.submit(runAll2);
			es.submit(runAll3);
			es.submit(runAll4);
			//Thread tfa = new Thread(runAll);//так пул потоков не запускается, это происходит через submit
			//tfa.start();
			es.shutdown();
			/*
			String text = "C:\\DANILA\\OOP - java\\Lab6\\First.txt";
			String text2 = "C:\\DANILA\\OOP - java\\Lab6\\Second.txt";
			String text3 = "C:\\DANILA\\OOP - java\\Lab6\\Third.txt";
			String text4 = "C:\\DANILA\\OOP - java\\Lab6\\Fourth.txt";
			String text5 = "C:\\DANILA\\OOP - java\\Lab6\\Fifth.txt";
			...
			texts[3] = text4;
			texts[4] = text5;
			BlockingQueue<Vehicle> bq = new ArrayBlockingQueue<Vehicle>(1);
			for (int i = 0; i<5; i++)
			{
				Runnable runText = new ThreadForText(texts[i], bq);
				Thread tft = new Thread(runText);
				tft.start();
			}
			for (int i = 0; i<5; i++)
			{
